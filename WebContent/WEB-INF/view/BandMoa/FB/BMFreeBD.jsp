...
<body style="overflow-x: hidden">
  ...
  <ul class="nav nav-tabs">
    ...
    <div style="width: 750px; margin: auto;">
	<form id="asdf" name="asdf" method="post" action="/BandMoa/CommentProc.do?seq=<%=CmmUtil.nvl(bDTO.getSeq()) %>">
	...
	<% if(session.getAttribute("SS_USER_ID") == null) {%>
	<input type="button" onclick="location.href='/BandMoa/ModifyCertifyTry.do?seq=<%=seq%>'" class="btn btn-primary btn-block" value="수정/삭제하기"><br>
	<%} else if (session.getAttribute("SS_USER_ID").equals("Admin")) { %>
	<input type="button" onclick="location.href='/BandMoa/ModifyCertifyTry.do?seq=<%=seq%>'" class="btn btn-primary btn-block" value="관리자 수정/삭제하기"><br>
	<%} else {%>
	<input type="button" onclick="location.href='/BandMoa/ModifyCertifyTry.do?seq=<%=seq%>'" class="btn btn-primary btn-block" value="수정/삭제하기"><br>
	<%} %>
	<a href="/BandMoa/FB/BMFreeB.do?Pno=1">돌아가기</a>
	</div>
	</div>
	</div>		

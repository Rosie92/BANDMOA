...
<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
    <%
  	List<BMQTDTO> bList = (List<BMQTDTO>)request.getAttribute("bList");
...
<body style="overflow-x: hidden">
    ...
    <div>
         <% for(BMQTDTO bDTO : bList) { %>
    <ul style=" padding:15px 0px 30px 0px; background-color: lightyellow;">
            <span style="; margin-left: 50px;">
                <%=CmmUtil.nvl(bDTO.getBoard_seq()) %>
            </span>
            <span style="margin-left:50px">
            <div style=" width:350px; text-align:center; display:inline-block;"><a href="/BandMoa/QT/BMqtBD.do?seq=<%=bDTO.getBoard_seq()%>"><%=CmmUtil.nvl(bDTO.getTitle()).replaceAll("<","&lt;").replaceAll(">","&gt;") %></a></div>
            </span>
	...
	<div class="tab-pane active" role="tabpanel" id="tab-1">
         ...
         <%
            if(paging.isPrev()==true) { %>
               <a class="page-item page-link" href="/BandMoa/QT/BMqtB.do?Pno=<%=paging.getStartPage()-3%>">이전</a>&nbsp;
         <%}   %>
         ...
         <%} else {%>
               <a class='pNum' href="/BandMoa/QT/BMqtB.do?Pno=<%=a%>">&ensp;<%=a%>&ensp;</a>
         <%}}%>
         <%
            if(paging.isNext()==true) { %>
               &nbsp;<a class="page-item page-link" href="/BandMoa/QT/BMqtB.do?Pno=<%=paging.getEndPage()+1%>">다음</a>
         <%}   %>
         </ul>
         </nav>
         </div>
	...
	<div>
	<ul class="nav nav-tabs">
   		<li class="nav-item"><a class="btn btn-primay" href="/BandMoa/QT/BMqtBR.do" role="button">글작성<span class="badge badge-pill badge-primary">★</span></a></li>
    </ul>
	</div>
	</div>

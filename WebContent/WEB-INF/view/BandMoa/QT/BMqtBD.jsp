...
<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
    <%
    BMQTDTO bDTO = (BMQTDTO)request.getAttribute("bDTO");
...
<body style="overflow-x: hidden">
  ...
  <ul class="nav nav-tabs">
      <li class="nav-item"><a class="nav-link active" role="tab" data-toggle="tab">팁게시판 <span class="badge badge-pill badge-primary">★</span></a></li>
  </ul><br>
    ...
    <ul style="background-color: lightyellow;">
            ...
            <a href="/BandMoa/CommentModifyTryQT.do?rno=<%=cDTO.getRno() %>">관리</a>
            <%} else { %>
            <a href="/BandMoa/CommentModifyTryQT.do?rno=<%=cDTO.getRno() %>">수정/삭제</a>
            <%} %>
            </div>
            </span>
    </ul>
    ...
    <div style="width: 750px; margin: auto;">
	<form id="asdf" name="asdf" method="post" action="/BandMoa/CommentProcQT.do?seq=<%=CmmUtil.nvl(bDTO.getSeq()) %>">
	...
	<% if(session.getAttribute("SS_USER_ID") == null) {%>
	<input type="button" onclick="location.href='/BandMoa/ModifyCertifyTryQT.do?seq=<%=seq%>'" class="btn btn-primary btn-block" value="수정/삭제하기"><br>
	<%} else if (session.getAttribute("SS_USER_ID").equals("Admin")) { %>
	<input type="button" onclick="location.href='/BandMoa/ModifyCertifyTryQT.do?seq=<%=seq%>'" class="btn btn-primary btn-block" value="관리자 수정/삭제하기"><br>
	<%} else {%>
	<input type="button" onclick="location.href='/BandMoa/ModifyCertifyTryQT.do?seq=<%=seq%>'" class="btn btn-primary btn-block" value="수정/삭제하기"><br>
	<%} %>
	<a href="/BandMoa/QT/BMqtB.do?Pno=1">돌아가기</a>
	</div>
	</div>
	</div>		

...
<body style="overflow-x: hidden">
    ...
    <div style="width: 740px; margin: left;">
            ...
            <div class="col-md-12">
            	<div>
                    <ul class="nav nav-tabs">
                        <li class="nav-item"><a class="nav-link active" role="tab" data-toggle="tab">댓글<span class="badge badge-pill badge-primary">★</span></a></li>
                    </ul><br>
  	    <div style="width: 750px; margin: auto; text-align:center;">
  	    <!-- 관리자 비밀번호 인증창 -->
  	    <%if(session.getAttribute("SS_USER_ID").equals("Admin")) { %>
		<form name="f" id="f" method="post" action="/BandMoa/CommentCertify2SR.do?rno=<%=rno%>">
		<input value="<%=rno%>" name="rno" hidden="hidden"><!-- 이거하면 서티파이2로 넘어갈것임 -->
        ...
        <a class="forgot" href="/BandMoa/SR/BMSrB.do?Pno=1" width="40%">돌아가기</a>
        
        <%} else {%>
        <!-- 사용자 비밀번호 인증창 -->
        <form name="f" id="f" method="post" action="/BandMoa/CommentCertifySR.do?rno=<%=rno%>">
		<input value="<%=rno%>" name="seq" hidden="hidden"><!-- 이거하면 서티파이2로 넘어갈것임 -->
        <h2 class="sr-only">Form</h2>
        <br><br><br><br><br>
              수정/삭제를 위해 <br>댓글 비밀번호를 입력해주세요.<br><br><br>
        <div class="form-group"><input class="form-control" type="password" name="pw" id="pw" style="width: 40%; margin-left:225px;"></div>
        <div class="form-group" id="submit"> <!-- submit이 input에 들어가면 유효성 검사가 실행이X -->
        <input class="btn btn-primary btn-block" style="cursor:pointer; width: 40%; margin-left:225px;" readonly="readonly" value="인증하기"></div>
        <a class="forgot" href="/BandMoa/SR/BMSrB.do?Pno=1" width="40%">돌아가기</a>
        <%} %>

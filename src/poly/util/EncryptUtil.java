...
public class EncryptUtil {
	 ...
	 * 
	 * 일반적인 암호화 알고리즘 SHA-256을 통해서만 암호화 시킬 경우, 암호화 된 값만 보고
	 * 일반적인 비밀번호에 대한 값을 쉽게 예측이 가능함 따라서,
	 * 암호화할 때 암호화되는 값에 추가적인 문자열을 붙여서 함께 암호화를 진행함
	 */
	
	final static String addMessage = "PolyDataAnalysis";//임의값
	
	/*
	 * AES128-CBC암호화 알고리즘에 사용되는 초기 백터와 암호화 키
	 */
	
	//초기 벡터 (16바이트 크기를 가지며, 16바이트 단위로 암호화시, 암호화할 총 길이가 16바이트가 되지 못하면 뒤에 추가하는 바이드)
	final static byte[] ivBytes = {0x00, 0x00, 0x00, 0x00, 0x00, 0x0, 0x00, 0x00, 0x00, 0x00, 0x00, 0x0, 0x00, 0x00, 0x00, 0x00 };
	 //AES128-CBC암호화 알고리즘에 사용되는 키 (16자리 문자만 가능함)
	...
	public static String encHashSHA256(String str) throws Exception{
		...
		try {
			 ...
			 * 여러 해시 알고리즘 중 가장 많이 사용되난 SHA-256를 지원하고있음
			 */
